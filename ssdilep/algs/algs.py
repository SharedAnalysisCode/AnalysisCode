...
class CutAlg(pyframe.core.Algorithm):
    ...
    #__________________________________________________________________________
    def cut_AtLeastTwo90GeVJets(self):
      count = 0
      for jet in self.store['jets_tight']:
        if jet.pt >= 90 * GeV:
          count += 1
    ...
    #__________________________________________________________________________
    def cut_AtLeastTwo80GeVJets(self):
      count = 0
      for jet in self.store['jets_tight']:
        if jet.pt >= 80 * GeV:
          count += 1
    ...
    #__________________________________________________________________________
    def cut_AtLeastTwo70GeVJets(self):
      count = 0
      for jet in self.store['jets_tight']:
        if jet.pt >= 70 * GeV:
          count += 1
    ...
    #__________________________________________________________________________
    def cut_AtLeastTwo60GeVJets(self):
      count = 0
      for jet in self.store['jets_tight']:
        if jet.pt >= 60 * GeV:
          count += 1
    ...
    #__________________________________________________________________________
    def cut_METtrkLow100(self):
      met = self.store["met_trk"]
      return met.tlv.Pt() < 100 * GeV
      ...
      if runNumber < 290000. :
        ...
        for i in xrange(self.chain.passedTriggers.size()):
          if self.chain.passedTriggers.at(i) in trigchains:
            for ele in self.chain['electrons_OSHN']:
    ...
    def cut_PassHLT(self):
        chain = ["HLT_e17_lhloose_nod0_mu14","HLT_mu26_ivarmedium","HLT_mu50","HLT_2e17_lhloose"]
        for i in xrange(self.chain.passedTriggers.size()):
            if self.chain.passedTriggers.at(i) in chain: return True
    ...
    def cut_PassORSingleLeptonTriggerMuon(self):
      ...
      for i in xrange(self.chain.passedTriggers.size()):
          if self.chain.passedTriggers.at(i) in chain:
            return True
      return False
    ...
    def cut_ExactlyZeroOSHNMuons(self):
        muons = self.store['muons_OSHN']
        if len(muons)==0: 
          return True
        return False
    ...
    def cut_ExactlyTwoOSHNElectrons(self):
        electrons = self.store["electrons_OSHN"]
        if len(electrons)==2: 
          return True
        return False
    ...
    def cut_OffDiagMixedEvent(self):
      if (len(self.store['electrons_loose_LooseLLH']) * len(self.store['muons'])) == 0:
        return False
    ...
    def cut_ExactlyThreeLooseMuon(self):
    ...
    def cut_TwoMuonTwoJetHT300(self):
        leptons = self.store['muons']
        jets = self.store['jets_tight']
        # jets.sort(key=lambda x: x.tlv.Pt(), reverse=True )
        assert jets[0].tlv.Pt() >= jets[1].tlv.Pt(), "jets not sorted.."
    ...
    def cut_bjetveto(self):
        ...
        jets = self.store['jets']
        for jet in jets:
          if jet.JvtPass_Medium and jet.fJvtPass_Medium and abs(jet.eta) < 2.5:
            if jet.isFix77:
              nbjets += 1
        if nbjets == 0:
          return True
    ...
    def cut_Mass60GeV2000muon(self):
    ...
    def cut_Mass300GeV400muon(self):
        electrons = self.store['muons']
        if len(electrons)==2 :
          tempMass = (electrons[0].tlv + electrons[1].tlv).M()
          if 300*GeV < tempMass <= 400*GeV :
    ...
    def cut_NoLooseFakesInMC(self):
      if "mc" not in self.sampletype:
        return True
      elif self.chain.mcChannelNumber in range(306538,306560) + range(302657,302713) + range(309063,309073) + range(306533,306561):
        return True
      electrons = self.store['electrons']
      muons = self.store['muons']
      for ele in electrons:
    ...
    def cut_atLeastOneLooseFakeInMC(self):
      if ("mc" not in self.sampletype):
        return False
...
class PlotAlgFFee(pyframe.algs.CutFlowAlg,CutAlg):
    ...
    def initialize(self):
        pyframe.algs.CutFlowAlg.initialize(self)
        # self.pt_bins  = [30., 35., 40., 45., 50., 55., 60., 65., 70., 75., 80., 90., 100., 120., 140., 180., 250., 350., 500., 10000.]
        self.pt_bins  = [30., 35., 45., 55., 65., 75., 90., 115., 140., 180., 250., 350., 500., 2000.]
        ...
        # self.trigger_strings   = ["HLT_e26_lhvloose_nod0_L1EM20VH","HLT_e60_lhvloose_nod0","HLT_e140_lhloose_nod0"]
        # self.trigger_bounds    = [70.                             ,800.                   ,99999999999]
        # self.trigger_prescaled = [111.53                          ,25.402                 ,1.0]
...
class PlotAlgCRele(pyframe.algs.CutFlowAlg,CutAlg):
    ...
    def __init__(self,
        ...
        self.mu_container = mu_container
        self.obj_keys = obj_keys
        self.array = array
    ...
    def plot(self, region, passed, list_cuts, cut, list_weights=None, weight=1.0):
        ...
        self.h_jet_lead_pt  = self.hist('h_jet_lead_pt', "ROOT.TH1F('$', ';p_{T}(j lead) [GeV];Events / (1 GeV)', 2000, 0.0, 2000.0)", dir=ELECTRONS)
        self.h_jet_lead_eta = self.hist('h_jet_lead_eta', "ROOT.TH1F('$', ';#eta(j lead);Events / (0.1)', 50, -2.5, 2.5)", dir=ELECTRONS)
        self.h_jet_lead_phi = self.hist('h_jet_lead_phi', "ROOT.TH1F('$', ';#phi(j lead);Events / (0.1)', 64, -3.2, 3.2)", dir=ELECTRONS)
        ...
        self.h_jet_sublead_pt =  self.hist('h_jet_sublead_pt', "ROOT.TH1F('$', ';p_{T}(j sublead) [GeV];Events / (1 GeV)', 2000, 0.0, 2000.0)", dir=ELECTRONS)
        self.h_jet_sublead_eta = self.hist('h_jet_sublead_eta', "ROOT.TH1F('$', ';#eta(j sublead);Events / (0.1)', 50, -2.5, 2.5)", dir=ELECTRONS)
        self.h_jet_sublead_phi = self.hist('h_jet_sublead_phi', "ROOT.TH1F('$', ';#phi(j sublead);Events / (0.1)', 64, -3.2, 3.2)", dir=ELECTRONS)
        ...
        if passed:
          ...
          # self.array += [(self.chain.runNumber,self.chain.mcEventNumber)]
          # self.f.write("%s\t%s\n" % (self.chain.runNumber,self.chain.eventNumber) )

          nbjets = 0
          njets  = 0
          for jet in self.store['jets']:
            if jet.JvtPass_Medium and jet.fJvtPass_Medium and abs(jet.eta) < 2.5:
              njets += 1
              if jet.isFix77:
                nbjets += 1
          ...
          if len(jets) > 1:
            ...
            self.h_jet_lead_phi.Fill(jets[0].tlv.Phi(), weight)
            self.h_jet_sublead_pt.Fill(jets[1].tlv.Pt()/GeV, weight)
            self.h_jet_sublead_eta.Fill(jets[1].eta, weight)
            self.h_jet_sublead_phi.Fill(jets[1].tlv.Phi(), weight)
          ...
          # print "----------------------"
          # if len(self.store[self.mu_container]) > 0:
          #   if electrons[0].truthType == electrons[1].truthType == 6:
          #     if electrons[0].truthOrigin + electrons[1].truthOrigin == 35:
          #       print "N mu"
          #     elif electrons[0].truthOrigin + electrons[1].truthOrigin == 36:
          #       print "N tau"
          #   else:
          #     print "fakes"
          # for ele in electrons:
          #   if hasattr(ele,'firstEgMotherTruthType'):
          #     continue
          #   print "==============="
          #   print ele.truthOrigin
          #   print ele.truthType
          #   print ele.trkcharge*-11.
          #   if hasattr(ele,'firstEgMotherTruthType'):
          #     print ele.firstEgMotherTruthType
          #     print ele.firstEgMotherTruthOrigin
          #     print ele.firstEgMotherPdgId
          #   print "==============="
...
class VarsAlg(pyframe.core.Algorithm):
    ...
    def execute(self, weight):
        ...
        # mJJ
        if len(jets_tight) > 1:
          self.store['MJJ'] = (jets_tight[0].tlv + jets_tight[1].tlv).M()/GeV
        else:
         self.store['MJJ'] = -999. 
        ...
        muons_OSHN = []
        for mu in muons:
          if mu.isIsolated_LooseTrackOnly and mu.tlv.Pt() > 25*GeV and abs(mu.tlv.Eta()) < 2.5 and (mu.isMedium or mu.isTight):
            muons_OSHN += [mu]
        self.store['muons_OSHN'] = muons_OSHN
        # OS HN ELECTRONS
        electrons_OSHN = []
        for ele in electrons:
          if ele.isIsolated_LooseTrackOnly and ele.tlv.Pt() > 25*GeV and ele.LHMedium:
            electrons_OSHN += [ele]
